...
import {safeUrl, getPage, robotsAllows, ldNodes, isEventType, typeOf,
        readMessage} from './_read.mjs';
...
export default async function handler(req, res) {
  ...
  if (action === 'probe') {
    ...
    try {
      ...
      out.robots = rob.why;
      if (!rob.ok) return res.status(200).json({...out, ok: false, verdict: 'robots.txt says no'});
      ...
      const events = ldNodes(page.body)
        .filter(o => isEventType(typeOf(o)) && o.startDate).length;
  ...
  if (action === 'inbox') {
    ...
    /* Checked against the same five words the column's constraint allows, so a
       typo narrows nothing rather than quietly returning an empty list. */
    const tri = ['ready','duplicate','needs-you','nothing','unread'].includes(req.body?.triage)
      ? `&triage=eq.${encodeURIComponent(req.body.triage)}` : '';
    const rows = await db('GET',
      `/rest/v1/inbox?select=*&order=received_at.desc&limit=200${want}${tri}`);
  ...
  /* ── read the links in a message ─────────────────────────────────────────
     The inbox stores what arrived and interprets nothing — that separation is
     deliberate and stays. This does not change it: it READS, it never writes.
     What comes back is a proposal a person accepts, the same contract
     /api/enrich has always had, minus the model and therefore minus the bill.

     Why this is the free half. A ticket page publishes schema.org, so a link
     needs no inference at all. A NEWSLETTER does not — its prose is where the
     events are, and pulling them out needs a model. So this reads what can be
     read for nothing, and says plainly when a message is the other kind. */
  if (action === 'inbox_read') {
    if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_KEY)
      return res.status(501).json({error: 'not_configured'});
    const n = Number(req.body?.id);
    if (!Number.isInteger(n) || n <= 0) return res.status(400).json({error: 'bad_id'});
    const [msg] = await db('GET', `/rest/v1/inbox?select=*&id=eq.${n}`);
    if (!msg) return res.status(404).json({error: 'no_such_message'});

    /* Re-reading is the point of the button: /api/inbox already did this on
       arrival, and by the time a person looks the answer may have moved —
       a duplicate may have been created since, or a venue registered. The
       stored copy is a cache of a question, never the answer itself. */
    const read = await readMessage(msg, db);
    const [row] = await db('PATCH', `/rest/v1/inbox?id=eq.${n}`,
      {read, triage: read.triage}, {Prefer: 'return=representation'});
    return res.status(200).json({ok: true, id: n, ...read, row});
  }
    ...
    if (on) {
      ...
      if (undated.length) return res.status(400).json({error: 'no_date',
        message: `${undated.length} event(s) have no date. Give them one or leave ` +
                 `them unpublished — the board is sorted by when things are on.`,
  ...
  if (action === 'create') {
    ...
    /* A row born here has by definition not been approved by anyone — pressing
       Create is the act of proposing it, not of publishing it. `published`
       defaults to TRUE in the database, so without this a listing created from
       the back office walks straight onto the board, which is the same hole
       `sync.py add` had. `places` has no such column and is untouched. */
    if (table === 'events' || table === 'activities') row.published = false;
